...
const Web3Provider = ({children}) => {
  ...
  const handleWallet = async () => {
    try {
      ...
      const {contractInstance,selectedAccount,chainId,networkName } = await getWeb3State();
      // Update the web3State with the fetched data
      setWeb3State({contractInstance,selectedAccount,chainId, networkName  });
      console.log(contractInstance,selectedAccount,chainId, networkName )
    } catch (error) {
      console.log(error); 
    }
  };
